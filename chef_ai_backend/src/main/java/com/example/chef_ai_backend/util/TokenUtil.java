...
public class TokenUtil {
    ...
    public String generateToken(Long userId) {
        try {
            ...
            log.info("为用户生成Token，userId：{}", userId);
            ...
            log.info("Token生成并存储成功，token：{}", base64Token);
            
            return base64Token;
        } catch (Exception e) {
            log.error("生成Token时发生异常，userId：{}", userId, e);
    ...
    public Long validateToken(String token) {
        try {
            ...
            if (userIdStr == null) {
                log.info("Token不存在或已过期，token：{}", token);
            ...
            log.info("Token验证成功，token：{}，userId：{}", token, userId);
    ...
    public void invalidateToken(String token) {
        try {
            if (token != null && !token.trim().isEmpty()) {
                ...
                log.info("Token注销成功，token：{}", token);
            }
        } catch (Exception e) {
            log.error("注销Token时发生异常，token：{}", token, e);
            throw new RuntimeException("Token注销失败：" + e.getMessage());
        }
    }
}