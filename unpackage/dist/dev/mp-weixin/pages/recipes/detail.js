...
var _detail = _interopRequireDefault(__webpack_require__(/*! ./pages/recipes/detail.vue */ 65));
// @ts-ignore
...
/* harmony import */ var _detail_vue_vue_type_template_id_198c1434___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./detail.vue?vue&type=template&id=198c1434& */ 66);
...
var render = function () {
  ...
  var l0 = _vm.__map(
    _vm.formatMethod(_vm.recipe.method),
    function (step, index) {
      var $orig = _vm.__get_orig(step)
      var g0 = step.endsWith("。")
      return {
        $orig: $orig,
        g0: g0,
      }
...
var _default = {
  ...
  methods: {
    ...
    formatMethod: function formatMethod(method) {
      if (!method) return [];
      return method.split('↵').filter(function (step) {
        return step.trim();
      }).map(function (step, index) {
        // 移除步骤中可能存在的额外序号（如第二个 1.）
        var cleanedStep = step.replace(/^\d+\.\s*/, '').trim();
        return cleanedStep;
      });
    },
    // 难度等级文本
    getDifficultyText: function getDifficultyText(level) {
      return level || '未知';
    },
    // 难度等级样式
    getDifficultyClass: function getDifficultyClass(level) {
